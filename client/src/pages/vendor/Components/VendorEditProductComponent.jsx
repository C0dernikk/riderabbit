...
import { setVendorVehicles } from "../../../features/vehicles/vehiclesSlice";
...
export default function VendorEditProductComponent() {
  ...
  const { vendorVehicles = [], metadata } = useSelector((state) => state.vehicles);
  const {
    models: modelData = [],
    brands: companyData = [],
  ...
  const onEditSubmit = async (editData) => {
    ...
    try {
      if (editData && vehicle_id) {
        ...
        await vendorService.updateVehicle(vehicle_id, formData);
        toast.dismiss(tostID);
        dispatch(
          setVendorVehicles(
            vendorVehicles.map((vehicle) =>
              vehicle._id === vehicle_id ? { ...vehicle, ...formData } : vehicle,
            ),
          ),
        );
