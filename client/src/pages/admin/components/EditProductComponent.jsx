...
export default function EditProductComponent() {
  ...
  const { userAllVehicles, metadata } = useSelector((state) => state.vehicles);
  const { models: modelData = [], brands: companyData = [], locations: locationData = [], districts: districtData = [] } = metadata;
  ...
  const onEditSubmit = async (editData) => {
    ...
    try {
      if (editData && vehicle_id) {
        ...
        const payload = {
          registrationNumber: editData.registeration_number,
          brand: editData.company,
          model: editData.model,
          title: editData.title,
          description: editData.description,
          price: editData.price,
          yearMade: editData.year_made,
          fuelType: editData.fuelType,
          seats: editData.Seats,
          transmission: editData.transmitionType,
          carType: editData.carType,
          location: editData.vehicleLocation,
          district: editData.vehicleDistrict,
        };
        await vehicleService.adminUpdateVehicle(vehicle_id, payload);
          ...
          >
            <div>
              <TextField
                ...
                defaultValue={updateingItem?.registrationNumber || ""}
              />

              <Controller
                control={control}
                name="company"
                defaultValue={updateingItem?.brand || ""}
                render={({ field }) => (
                  <TextField
                    {...field}
                    required
              ...
              <TextField
                ...
                InputLabelProps={{ shrink: true }}
                {...register("name")}
                defaultValue={updateingItem?.title || ""}
              />
              ...
              <TextField
                ...
                InputLabelProps={{ shrink: true }}
                {...register("year_made")}
                defaultValue={updateingItem?.yearMade || ""}
              />
              <Controller
                control={control}
                name="fuelType"
                defaultValue={updateingItem?.fuelType || ""}
                render={({ field }) => (
                  <TextField
                    {...field}
                    required
            ...
            <div>
              <Controller
                name="carType"
                control={control}
                defaultValue={updateingItem?.carType || ""}
                render={({ field }) => (
                  <TextField
                    {...field}
                    required
              ...
              <Controller
                ...
                defaultValue={updateingItem?.transmission || ""}
                render={({ field }) => (
                  <TextField
                    {...field}
                    required
              ...
              <TextField
                ...
                placeholder="e.g. Well-maintained car with excellent mileage. Features include AC, Bluetooth audio, and power windows."
                InputLabelProps={{ shrink: true }}
                defaultValue={updateingItem?.description || ""}
                multiline
                rows={4}
                sx={{
                  width: "100%",
